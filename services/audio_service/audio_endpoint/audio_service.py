...
def send_file(input_file, output_file):
    ...
    ssh.connect(config['FILE_SERVER']['IP'], port=int(config['FILE_SERVER']['PORT']), username=config['FILE_SERVER']['USERNAME'], key_filename='id_rsa')
    sftp = ssh.open_sftp()
...
def record_work_day(cards):
    while True:
        ...
        for file in ray.get(recording_processes):
            send_file(config["ENV"]["DATA_DIR"]+file, )
